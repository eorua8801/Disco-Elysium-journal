...
export function CheckCard({ check }: CheckCardProps) {
  ...
  const T = useT();
  const diffLabel = T.dice.difficulties[check.difficulty] ?? getDifficultyLabel(check.difficulty);
  const skillName = T.skills.names[check.skillId] ?? skill?.name ?? check.skillId;
